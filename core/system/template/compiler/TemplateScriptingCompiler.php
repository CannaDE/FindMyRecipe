...
use fmr\system\template\compiler\plugins\CompilerTemplatePluginInterface;
use fmr\system\template\compiler\plugins\PrefilterTemplatePluginInterface;
...
class TemplateScriptingCompiler {
     ...
     * @var CompilerTemplatePluginInterface[]
     */
    ...
    protected function compileCompilerPlugin($tagCommand, $tagArgs)
    {
        ...
        if (!isset($this->compilerPlugins[$className])) {
            ...
            if (!($this->compilerPlugins[$className] instanceof CompilerTemplatePluginInterface)) {
                throw new TemplateCompilerException(
                    static::formatSyntaxError(
                        "Compiler plugin '" . $tagCommand . "' does not implement the interface 'CompilerTemplatePluginInterface'",
                        $this->currentIdentifier
                    )
                );
            }
    ...
    protected function compileCaptureTag($startTag, $captureTag = null)
    {
        if ($startTag) {
            ...
            $phpCode = "<?php\n";
            $phpCode .= "\$this->tplVars['tpl']['capture'][" . $capture['name'] . "] = ob_get_clean();\n";
            if (!empty($capture['variable'])) {
                $phpCode .= "\$this->" . ($capture['append'] ? 'append' : 'assign') . "(" . $capture['variable'] . ", \$this->tplVars['tpl']['capture'][" . $capture['name'] . "]);\n";
            }
            $phpCode .= "?>";
    ...
    protected function compileSectionTag($sectionTag)
    {
        ...
        $sectionProp = "\$this->tplVars['tpl']['section'][" . $args['name'] . "]";
        ...
        $phpCode .= "\$this->tplVars[" . $args['name'] . "] = " . $sectionProp . "['index'];\n";
    ...
    protected function compileForeachTag($foreachTag)
    {
        ...
        $foreachProp = '';
        if (isset($args['name'])) {
            $foreachProp = "\$this->tplVars['tpl']['foreach'][" . $args['name'] . "]";
        }
        ...
        $itemVar = \mb_substr($args['item'], 0, 1) != '$' ? "\$this->tplVars[" . $args['item'] . "]" : $args['item'];
        ...
        if (isset($args['key'])) {
            $keyVar = \mb_substr($args['key'], 0, 1) != '$' ? "\$this->tplVars[" . $args['key'] . "]" : $args['key'];
    ...
    protected function compileIncludeTag($includeTag, $identifier, array $metaData)
    {
        ...
        // check arguments
        if (!isset($args['file'])) {
            $formatError = static::formatSyntaxError(
                'missing file tag in include tag',
                $this->currentIdentifier,
                $this->currentLineNo
            );
            throw new TemplateCompilerException(
        ...
        if ($staticInclude) {
            ...
            if (!empty($args)) {
                foreach ($args as $variable => $value) {
                    if (\substr($value, 0, 1) == "'") {
                        // string values
                        $phpCode .= "\$this->tplVars['" . $variable . "'] = " . $value . ";\n";
                    } else {
                        if (\preg_match('~^\$this->tplVars\[\'(.*)\'\]$~U', $value, $matches)) {
        ...
        $phpCode = "<?php\n";
        if ($once) {
            $phpCode .= "if (!isset(\$this->tplVars['tpl']['includedTemplates'][" . $file . "])) {\n";
        }
        $hash = StringUtil::getRandomID();
        $phpCode .= "\$outerTemplateName" . $hash . " = \$this->tplVars['tpl']['template'];\n";
        ...
        $phpCode .= "\$this->tplVars['tpl']['template'] = \$outerTemplateName" . $hash . ";\n";
        $phpCode .= "\$this->tplVars['tpl']['includedTemplates'][" . $file . "] = 1;\n";
        if ($once) {
            $phpCode .= "}\n";
        }
        $phpCode .= '?>';
    ...
    public function parseTagArgs($tagArgs, $tag)
    {
        ...
        if (!\preg_match('~^(?:\s+\w+\s*=\s*[^=]*(?=\s|$))*$~s', $tagArgs)) {
            throw new TemplateCompilerException(
                static::formatSyntaxError(
                    'syntax error in tag <strong>{' . ($tag ? 'elseif' : 'if') . '}</strong>',
                    $this->currentIdentifier,
                    $this->currentLineNo
                )
            );
        ...
        for ($i = 0, $j = \count($matches[1]); $i < $j; $i++) {
            ...
            // reinserts strings
            foreach (StringStackUtil::getStack('singleQuote') as $hash => $value) {
                if (\str_contains($string, $hash)) {
                    $string = \str_replace($hash, $value, $string);
                }
            }
            foreach (StringStackUtil::getStack('doubleQuote') as $hash => $value) {
                if (\str_contains($string, $hash)) {
                    $string = \str_replace($hash, $value, $string);
                }
            }
    ...
    public static function formatSyntaxError($errorMsg, $file = null, $line = null)
    {
        ...
        if ($file && $line) {
            $errorMsg = sprintf("%s in template <strong>%s</strong>:<strong>%s</strong>", $errorMsg, $file, $line);
        } elseif ($file && !$line) {
            $errorMsg = sprintf(" in template <strong>%s</strong>", $file);
        }
    ...
    protected function compileIfTag($tagArgs, $elseif = false)
    {
        ...
        for ($i = 0, $j = \count($values); $i < $j; $i++) {
            ...
            if ($operator !== '!' && $values[$i] == '') {
                $formatError = static::formatSyntaxError(
                    'syntax error in tag <strong>{' . ($elseif ? 'elseif' : 'if') . '}</strong>',
                    $this->currentIdentifier,
                    $this->currentLineNo
                );
                throw new TemplateCompilerException(
    ...
    protected function compileOutputTag($tag)
    {
        ...
        if (\preg_match('~^(RELATIVE_)?([A-Z]+)_DIR$~', $tag, $matches)) {
            throw new TemplateCompilerException("Disallowed constant","Accessing internal constant '" . $tag . "' is disallowed, please use '\$__core->getPath()' instead");
        }
        ...
        if ($encodeHTML) {
            $parsedTag = 'fmr\util\StringUtil::encodeHTML(' . $parsedTag . ')';
        }
        ...
        elseif ($formatNumeric) {
            $parsedTag = 'fmr\util\StringUtil::formatNumeric(' . $parsedTag . ')';
        }
    ...
    protected function compileSimpleVariable($variable, $type = '', $allowConstants = true)
    {
        ...
        if ($type == 'variable') {
            return '$this->tplVars[\'' . \substr($variable, 1) . '\']';
        } elseif ($type == 'string') {
            return $variable;
        } elseif (
            $allowConstants
                ...
                switch ($status) {
                    ...
                    case 'modifier':
                        ...
                        if (\class_exists($className)) {
                            ...
                            $formatError = static::formatSyntaxError(
                                "unknown modifier '<strong>" . $values[$i] . "</strong>'",
                                $this->currentIdentifier,
                                $this->currentLineNo
                            );
                            throw new TemplateCompilerException(
                            ...
                            $formatError = static::formatSyntaxError(
                                "function '<strong>" . $values[$i] . "</strong>' may not be called within a template",
                                $this->currentIdentifier,
                                $this->currentLineNo
                            );
                            throw new TemplateCompilerException(
            ...
            if ($operator !== null) {
                switch ($operator) {
                    ...
                    case '(': // left parenthesis
                        ...
                        $formatError = static::formatSyntaxError(
                            "unexpected '->' in tag '<strong>" . $tag . "</strong>'",
                            $this->currentIdentifier,
                            $this->currentLineNo
                        );
    ...
    public function applyPrefilters($templateName, $string)
    {
        foreach ($this->template->getPrefilters() as $prefilter) {
            ...
            if ($prefilter instanceof PrefilterTemplatePluginInterface) {
    ...
    {
        return StringStackUtil::reinsertStrings($string, 'literal');
    }
    ...
    {
        return StringStackUtil::pushToStringStack($matches[1], 'literal');
    }
    ...
    {
        return StringStackUtil::pushToStringStack($matches[0], 'singleQuote');
    }
    ...
    private function replaceDoubleQuotesCallback($matches)
    {
        ...
        return StringStackUtil::pushToStringStack($matches[0], 'doubleQuote');
    }
    ...
    {
        $string = StringStackUtil::reinsertStrings($string, 'singleQuote');

        return StringStackUtil::reinsertStrings($string, 'doubleQuote');
    }
     ...
     * @return  string
     */
    private function replaceConstantsCallback(array $matches): string {
        return StringStackUtil::pushToStringStack($matches[1], 'constants');
    }
     ...
     * @return  string
     */
    public function reinsertConstants(string $string): string {
        return StringStackUtil::reinsertStrings($string, 'constants');
    }
