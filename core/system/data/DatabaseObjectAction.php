...
use fmr\system\exception\PermissionDeniedException;
use fmr\system\exception\SystemException;
use fmr\system\exception\UserInputException;
 ...
 */
abstract class DatabaseObjectAction implements DatabaseObjectActionInterface, DeleteActionInterface {
    /**
     * pending action
     * @var string
     */
     ...
     * @var string[]
     */
    protected array $allowGuestAccess = [];
    ...
    public function validateAction()
    {
        ...
        if (TimeMonitoring::getUser()->id === 0 && !in_array($this->getActionName(), $this->allowGuestAccess)) {
        ...
        if (!method_exists($this, $actionName)) {
            throw new AjaxException(sprintf("<strong>%s</strong> has not implemented method <strong>%s</strong>", $this::class, $actionName), 400);
        }
    ...
    public function validateCreate()
    {
        ...
        if (\is_array($this->permissionsCreate) && !empty($this->permissionsCreate)) {
            //TimeMonitoring::getSession()->checkPermissions($this->permissionsCreate);
        } else {
            throw new PermissionDeniedException();
        }
    }
        ...
        switch ($type) {
            ...
            case self::TYPE_JSON:
                if (!isset($target[$variableName])) {
                    ...
                    try {
                        $target[$variableName] = json_decode($target[$variableName]);
